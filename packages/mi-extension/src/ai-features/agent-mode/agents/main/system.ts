...
- Some skills are user-invocable only (they never appear in \`# Available Skills\`); you cannot activate these with ${SKILL_TOOL_NAME}. If one would help (e.g. a user-only skill you saw activated earlier), do not call ${SKILL_TOOL_NAME} for it — instead ask the user to invoke it themselves by typing \`/skill-name\` at the start of their next message.
