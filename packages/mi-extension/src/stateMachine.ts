...
// KNOWN LIMITATION (pre-existing): the 'projectType'/'displayOverview' keys below are written to the single window-global `workspaceState`, so in a multi-root workspace the last-detected project's values win for every project.
