...
export async function createTests(uri: Uri) {
    ...
    for (let i = 0; i < relativePath.length; i++) {
        ...
        // Suites always live directly under src/test/wso2mi/<suite>.xml, so this
        // level is always the same and adds no useful grouping — list suites
        // directly under the project node instead of nesting under it.
