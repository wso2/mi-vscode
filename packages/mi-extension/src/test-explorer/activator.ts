...
export async function createTests(uri: Uri) {
    ...
    // Ensure the project appears as a top-level node
    ...
    for (let i = 0; i < relativePath.length; i++) {
        ...
        // Test suites always live directly under src/test/wso2mi/<suite>.xml
        if (!isLeaf && i === 0 && level === "wso2mi") {
            continue;
        }
 ...
 * is shown in the test explorer even when it has no test suites yet.
 */
...
function getOrCreateProjectNode(projectRoot: string, projectName: string): TestItem {
    ...
    } else if (projectNode.label !== projectName) {
        projectNode.label = projectName;
    }
    return projectNode;
}
...
export async function resetTestStateNodes() {
    testDirNodes.length = 0;
