...
export class UnitTest {
    ...
    public async openUnitTestFormByMainBtn() {
        ...
        // The project node is always shown in the Test Explorer, even with zero
        // tests, so the empty-view welcome content is never rendered. Use the
        // node's inline "Add MI Unit Test" action instead. Match on the project
        // name prefix only, not the run state suffix (e.g. "Not yet run"), since
        // that suffix changes once a test under this project has been executed.
    ...
    private unitTestTreePath(name: string): string[] {
        // The test explorer nests suites directly under the project node: Project > Suite.
        return [`${this.projectName} (Not yet run)`, `${name} (Not yet run)`];
    }
