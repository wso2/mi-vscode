...
export function ProjectAddPanel(props: ProjectAddPanelProps) {
    ...
    const fetchWorkspaceProjects = async () => {
        try {
            ...
            // Hide projects already present as a dependency
    ...
    const handleSelectPom = async () => {
        ...
        try {
            ...
            console.error("Failed to extract maven coordinates from the selected pom.xml", e);
            rpcClient.getMiVisualizerRpcClient().showNotification({
                message: "Failed to read the selected pom.xml file.",
                type: "error"
            });
        }
    };
