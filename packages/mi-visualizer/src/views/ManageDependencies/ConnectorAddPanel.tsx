...
interface ConnectorAddPanelProps {
    ...
    // When true, this panel is scoped to inbound connector modules (mi-inbound-* artifacts):
    // the zip import goes into the inbound-connectors folder and labels reflect that scope.
    inboundOnly?: boolean;
}
...
export function ConnectorAddPanel(props: ConnectorAddPanelProps) {
    ...
    const handleDownloadConnector = async (connector: any) => {
        ...
        try {
            ...
            // Allow the saved pom to be served to the language server before downloading.
    ...
    return (
        <>
            <HeaderBar>
                ...
                <ActionButtons>
                    <Button appearance="secondary" onClick={() => handleImportZip(!!inboundOnly)}>
                        <Codicon name="file-zip" />&nbsp;{inboundOnly ? "Import Inbound Connector" : "Import Connector"}
                    </Button>
            ...
            <TextField
                placeholder={inboundOnly ? "Search inbound connectors" : "Search connectors"}
                value={searchValue}
            ...
            {connectors === undefined ? (
                ...
                <Typography sx={{ marginTop: '10px', opacity: 0.7 }}>
                    No {inboundOnly ? "inbound connectors" : "connectors"} available to add. Use "Import {inboundOnly ? "Inbound Connector" : "Connector"} (.zip)" or "Configure Dependency".
                </Typography>
            ) : (
                <div>
                    {availableConnectors.map((connector: any) => {
                        // Inbound connectors are always added at the latest version — no picker.
